...
const WebsiteManager = () => {
  ...
  const validateWebsiteName = (name) => {
    ...
    const existingWebsite = websites.find(website => 
      website.name.toLowerCase().trim() === name.toLowerCase().trim() && 
      website._id !== editingId &&
  ...
  const validateDomain = (domain) => {
    ...
    const existingWebsite = websites.find(website => 
      website.domain.toLowerCase().trim() === domain.toLowerCase().trim() && 
      website._id !== editingId &&
      !website.deleted
    );
  ...
  const handleDelete = (websiteId) => {
    ...
    setDeletePopup({
      ...
      message: isAlreadyDeleted
        ? "This website is already marked as deleted. Click OK to permanently remove it from the database. This action cannot be undone."
        : "This will mark the website as inactive and add a deleted flag. Click OK to continue.",
  ...
  const handleRevertConfirm = async () => {
    ...
    try {
      ...
      const existingWebsite = websites.find(w => 
        w._id !== websiteId &&
        w.domain.toLowerCase().trim() === website.domain.toLowerCase().trim() &&
        !w.deleted
      );
  ...
  return (
    <div className="paddingAll20">
      <PageHeader
        ...
        subtitle="Manage multiple websites for multi-tenant support"
        isEditing={!!editingId}
        editText="Edit Website"
        createText="Add New Website"
      />
        ...
        <form onSubmit={handleSubmit} className="brandForm">
          <div className="makeFlex row gap10">
            <div className="fullWidth">
              <FormField
                ref={websiteNameInputRef}
                type="text"
                name="name"
                label="Website Name"
                value={formData.name}
                onChange={handleChange}
                placeholder="Enter Website Name (e.g., PhotuPrint Main Site)"
                required={true}
              />
            </div>
          </div>
          ...
          <div className="makeFlex row gap10">
            <div className="fullWidth">
              <FormField
                ...
                onChange={handleChange}
                placeholder="Enter domain (e.g., photuprint.com)"
                required={true}
                info="Domain will be automatically converted to lowercase"
              />
            </div>
          </div>
          ...
          <div className="formActions paddingTop16">
            <button type="submit" disabled={loading} className="btnPrimary">
              {loading ? (
                ...
                <span>{editingId ? "Update Website" : "Add Website"}</span>
              )}
            </button>
        ...
        <div className="listHeader makeFlex spaceBetween end appendBottom24">
          <div className="leftSection">
            ...
            <StatusFilter
              ...
              counts={calculateStandardStatusCounts(websites)}
              disabled={loading}
            />
          </div>
                ...
                {displayedCards.map((website) => (
                  <EntityCard
                      ...
                      return (
                        <>
                          ...
                          <div className="brandDetail makeFlex spaceBetween alignCenter paddingTop8 paddingBottom8">
                            ...
                            <span className={`detailValue font14 ${website.deleted ? 'deleted' : (website.isActive ? 'greenText' : 'inactive')} appendLeft6`}>
                              {website.deleted ? 'Deleted' : (website.isActive ? 'Active' : 'Inactive')}
                            </span>
                          </div>
                        </>
                      );
                    ...
                    renderActions={(website) => (
                      <ActionButtons
                        onEdit={website.deleted ? undefined : () => handleEdit(website)}
                        onDelete={() => handleDelete(website._id || website.id)}
                        onRevert={website.deleted ? () => handleRevert(website._id || website.id) : undefined}
                        loading={loading}
                        size="normal"
                        editText="✏️ Edit"
                        deleteText={website.deleted ? "🗑️ Final Del" : "🗑️ Delete"}
                        revertText="🔄 Undelete"
                        editTitle="Edit Website"
                        deleteTitle={website.deleted ? "Final Del" : "Delete Website"}
                    ...
                    <tbody>
                      {currentWebsites.map((website) => (
                        <tr key={website._id || website.id} className="tableRow">
                          ...
                          <td className="tableCell">
                            <span className={`statusText ${website.deleted ? 'deleted' : (website.isActive ? 'active' : 'inactive')}`}>
                              {website.deleted ? 'Deleted' : (website.isActive ? 'Active' : 'Inactive')}
                            </span>
                          </td>
                          <td className="tableCell">
                          ...
                          <td className="tableCell">
                            <div className="tableActions makeFlex gap8">
                              <ActionButtons
                                ...
                                size="small"
                                editText="✏️"
                                deleteText={website.deleted ? "🗑️" : "🗑️"}
                                revertText="🔄"
                                editTitle="Edit Website"
                                deleteTitle={website.deleted ? "Final Del" : "Delete Website"}
                                revertTitle="Restore Website"
